...
df_agg_sim$FR_floorIdx[1]   <- with(df_agg_sim, MA[1] / AL_floorIdx[1])
...
penSim_results %>% 
	filter(sim == 0) %>% 
	select(runname, sim, year, AL_fullIdx, NC_PR, C_PR, FR_fullIdx, PR, ben_idx, SC_legacy, C, ERC, SC_legacy, NC, SC, LG) %>% 
	print()
