...
# results_all %>%
# 	filter(runname %in% c("baseline", "DA"), sim == 0, year <= 40) %>%
# 	select(runname, sim, year, AL, FR_MA, FR_fullIdx, NC, SC, C, ERC, SC_legacy, cola_actual) %>%
# 	as_tibble
...
df_PVC_40y <- get_PVC(results_stch, 40)
df_PVC_30y <- get_PVC(results_stch, 30)
df_PVC_15y <- get_PVC(results_stch, 15)
...
# # 	DA                       = "Conditional \nindexation" 
# )
# #runnames_pvERC_p1 <- names(runnames_pvERC_p1)
# 
# 
#   
# runnames_pvERC_p2 <- c(
...
# 	DA                       = "Conditional \nindexation" 
# )
#runnames_pvERC_p2 <- names(runnames_pvERC_p2)




# fig_title    <- paste0("Distributions of the present value ", nyear_pvERC, "-year employer contribution (ERC) \nunder DB-based risk-sharing policies") 
#   fig_dist_pvERC_y30_p1 <- 
#   df_PVC_qtiles_plot %>% 
#   filter(runname %in% names(runnames_pvERC_p1)) %>% 
# 	mutate(runname = factor(runname, levels = names(runnames_pvERC_p1), 
# 													         labels = runnames_pvERC_p1)) %>% 
# 	ggplot(aes(x = runname)) + 
# 	geom_boxplot(width = 0.4,
# 							 stat = "identity",
# 							 aes(ymin   = ERC_PV_q10,
...
# 
# fig_dist_pvERC_y30_p1
# save_figure(fig_dist_pvERC_y30_p1, dir_outputs, w = 10, h = 10, scale = 0.65)
# 
# 
# fig_title    <- paste0("Distributions of the present value ", nyear_pvERC, "-year employer contribution (ERC) \nunder more-complex risk-sharing policies") 
#   fig_dist_pvERC_y30_p2 <- 
#   df_PVC_qtiles_plot %>% 
#   filter(runname %in% names(runnames_pvERC_p2)) %>% 
# 	mutate(runname = factor(runname, levels = names(runnames_pvERC_p2), 
# 													         labels = runnames_pvERC_p2)) %>% 
# 	ggplot(aes(x = runname)) + 
# 	geom_boxplot(width = 0.4,
# 							 stat = "identity",
# 							 aes(ymin   = ERC_PV_q10,
...
#  
# fig_dist_pvERC_y30_p2
# save_figure(fig_dist_pvERC_y30_p2, dir_outputs, w = 10, h = 10, scale = 0.65)
# 
# 
...
tab_pvERC <- 
	df_tab_ERC_PV %>% 
	select(grp_CAGR40, Return_Range, ERC_PV_baseline, one_of(names(runnames_pvERC_tbl))) %>% 
  gather(policy, value, -grp_CAGR40, -Return_Range) %>% 
	mutate(policy = factor(policy, levels = c("ERC_PV_baseline", runnames_cost2),
												         labels = c("PV employer cost\nin Baseline", runnames_cost2_labels)),
												 ) %>%
	spread(policy, value)
ncol_tab <- ncol(tab_pvERC)
...
tbl_pvERC_y15 <- get_pvERC_tbl(df_PVC_15y)
tbl_pvERC_y30 <- get_pvERC_tbl(df_PVC_30y)
# tbl_pvERC_y40 <- get_pvERC_tbl(df_PVC_40y)
