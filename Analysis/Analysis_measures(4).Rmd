...
* Comparing ERC rates with baseline in a deterministic asset shock scenarios. 
...
## Impact on contribution volatility
...
tab_ERCMaxChg_qtiles %>% 
	select(qtile, one_of(policy_select)) %>% 
...
## Impact of policy parameters




```{r sim2sim_prep, eval=FALSE, include=FALSE}
### EXPLORE
...
```{r sim2sim_1, eval=FALSE, include=FALSE}
### EXPLORE
...
tab_bySim_ERCptiles %>% knitr::kable(digits = 5)
...
df_benefit_qtile_y1 %>% 
	...
	spread(runname, values) 
tab_benefit1 %>% knitr::kable(digits = 2)
...
df_benefit_qtile_y1 %>% 
	...
	spread(runname, values) 
tab_benefit2 %>% knitr::kable(digits = 2)
