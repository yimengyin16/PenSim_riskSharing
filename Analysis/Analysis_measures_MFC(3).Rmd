...
df_sum_PVB_net %<>% 	
	left_join(filter(df_sum_PVB_net, runname == "baseline_FR100") %>% 
							ungroup %>% 
...
df_sum_PVC <-
	df_PVC %>% 
	select(runname, sim, ERC_PV) %>% 
	left_join(filter(df_PVC, runname == "baseline_FR100") %>% ungroup %>% select(sim, ERC_PV_baseline = ERC_PV), by = "sim" ) %>% 
  left_join(df_CAGR40, by = "sim") %>% 
	mutate(ERC_PV          = 100 * ERC_PV / df_PVC_sim2sim$baseline[df_PVC_sim2sim$Percentile == "ptile50"], 
		     ERC_PV_baseline = 100 * ERC_PV_baseline / df_PVC_sim2sim$baseline[df_PVC_sim2sim$Percentile == "ptile50"],
		     ERC_PV_diff     = ERC_PV - ERC_PV_baseline,
...
# runnames_show_sum <- c("baseline_FR100", "cola_return_FR100", "cola_FR_FR100", "EEC_return_FR100", "EEC_FR_FR100")  
# df_sum_comparePV_net %>% 
# 	ungroup %>% 
