...
1. Sim-to-sim comparisons. Calculate sim-to-sim differences for each sim then examine:
 - Order the differences by an external variable (eg. variable values in baseline, coumpund return), then present the quantiles
 - Order the differences by an external variable (eg. variable values in baseline, coumpund return), and divide them into serveral groups, then present the median/mean of each group.  
...
- Removing shared-ADC and FR ramp from figures for the MFC paper
- CQ2 M1: just show probs for a single year (y10 or 20?)
- Maybe:
  - Show upside and downside risks, eg EEC_return policies 
- CQ2 M2/3: make it into quintile groups
- Benefit analysis: using an individual as example (year of death known?)
- BQ2-M1 to table
- Make table format consistent wherever possible
- plot for trade-off: protection for ER vs EE
...
2. **Median sim-to-sim differences between PVC under baseline and PVC under risk sharing policy (40-year) in groups of 40-year compound return. Calculate the differences in PVC for each of the 2,000 simulations, and present average differences within 5 groups of 40-year CAGR. (can be presented as differences in normalized dollar values or differences percentage). 
...
4. *Percentiles of the PVC under each policy. This measure is NOT a sim-to-sim comparison, but a comparison of PVC distributions. (distribution of differences vs. difference in distributions.)
...
1. Probability of ERC rate rising by more than 10 percentage points within 5 years. This measures the the likelihood of large increases of ERC
2. Max increase in ERC rate in 5 years. This measures the strength of the buffering effects when asset shock occurs.  
...
## Measure 1, Sim-to-sim comparison of PVC at the same percentile PVC under baseline 
...
## Measure 2, Sim-to-sim differences in 40-year compound return groups
...
**Cost Q1, Measure 2**: Present value of ERC over 40 years compared to baseline: median differences in 5 return groups
...
TODO: Adding median baseline PV ERC 
...
**Cost Q1, Measure 4** Distribution of PV ERC within eachpolicy
...
**Cost Q2, Measure 1** Measures the likelihood of large increases in ERC: Probability of ERC rate rising by more than 10 percentage points within 5 years. 
...
**Cost Q2, Measure 2** Measures to what extent risk-sharing policies can buffer the impact of asset shock occurs: Max increase in ERC rate in 5 years.  
...
1. Two probability based measures for the risk of low overall benefit:
	- 1.1* Risk of actual overall benefit significantly lower than expected(baseline): Prob of PV benefit is below 90% of PV benefit under baseline
...
1. Measures for the risk of experiencing low benefit sometime during retirement
	- 1.1 Chance of experiencing low benefit at some point: Prob of real benefits falling below 90% of starting benefit in any year up to a given year
...
2. Measures for large decline of benefit in a short period of time
	...
	- 2.2 Percentiles of max (or 95th ptile) decrease in real benefit in 5-years for a single cohort: percentiles (Tail-risks)

3. Range of real benefit level at age 80. Supplemental to other measures. Easier to see the tail risk: how low the real benefit can become (10th percentile)?
...
- 1.1 Risk of failing to protect against inflation: Prob of PV benefit is below 90% of PV benefit with full inflation protection
- 1.2 Risk of actual overall benefit significantly lower than expected(baseline): Prob of PV benefit is below 90% of PV benefit under baseline
