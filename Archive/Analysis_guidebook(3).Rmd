...
# df_ERCMaxChg_30y$ERCMaxChg
# 
# 
# 
# results_all %>%
# 	filter(runname %in% c("baseline", "DA"), sim == 12, year <= 40) %>%
# 	select(runname, sim, year, AL, FR_MA, FR_fullIdx, FR_floorIdx, SC, C,NC_PR, ERC_PR, SC_legacy, cola_actual) %>%
# 	as_tibble
# 
# 
# df_ERCMaxChg_30y$ERCMaxChg %>% 
#     filter(runname %in% c("baseline", "DA")) %>% 
# 	  select(runname, sim, ERC_PR_chg5yMax) %>% 
# 	  spread(runname, ERC_PR_chg5yMax) %>% 
# 	  mutate(baselineLarger = baseline > DA) %>% 
# 	  pull(baselineLarger) %>% sum
# 
# df_ERCMaxChg_30y$ERCMaxChg %>% 
#     filter(runname %in% c("baseline", "DA"), sim == 12) 
# 
# # DA reduces max 5-year change compared to baseline only in less than 1/4 sims (<500 sims out of 2,000)
# 
# results_all %>%
# 	filter(runname %in% c("baseline", "DA"), sim == 100, year <= 40) %>%
# 	select(runname, sim, year, ERC_PR) %>% 
# 	ggplot(aes(x = year, y = ERC_PR, color = runname)) + 
# 	geom_line() +
# 	geom_point()
...
# 
# df_PVC_qtiles_plot <- df_PVC_qtiles_y80_plot 
# nyear_pvERC <- 80
# 
# fig_title    <- paste0("Distributions of the present value of ", nyear_pvERC, "-year employer contributions (ERC) \nunder different risk-sharing policies") 
...
save_as_image(tbl_pvERC_y15, path = paste0(dir_outputs, "tbl_pvERC_y15.png"))
save_as_image(tbl_pvERC_y30, path = paste0(dir_outputs, "tbl_pvERC_y30.png"))
...
save_as_image(tbl_ERCvol_y15, path = paste0(dir_outputs, "tbl_ERCvol_y15.png"))
save_as_image(tbl_ERCvol_y30, path = paste0(dir_outputs, "tbl_ERCvol_y30.png"))
