...
fn <- function(df){
  for(j in 2:41){
	  df$DC_balance[j] <- with(df, (DC_balance[j - 1] - B_DC[j - 1]) * (1 + i.r[j - 1]))
	  df$B_DC[j] <-  with(df, DC_balance[j] * withdrawalRate )
  }
df	
}
...
results_all %<>% 
	left_join(select(df_hybrid, sim, year, B_DC, DC_balance), by = c("sim", "year")) %>% 
	mutate(B_DC = ifelse(str_detect(runname, "hybrid"), B_DC, 0),
				 DC_balance = ifelse(str_detect(runname, "hybrid"), DC_balance, 0)
				 )
