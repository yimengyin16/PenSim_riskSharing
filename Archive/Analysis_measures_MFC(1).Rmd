...
- check cola_return policy
  -  Check PV cost with 100% funded ratio
...
df_assetShock_plot <- 
  df_assetShock %>% 
	filter(sim == -2, year <=30, runname %in% runnames_show_det) %>% 
	mutate(SC_PR = SC / PR) %>% 
...
df_assetShock_plot %>% 
  ...
  coord_cartesian(ylim = c(0.5, 1))	+
	...
	scale_color_manual(values = c("black", brewer_pal(type = "qual", palette = "Set2")(6) ))
