...
dir_outputs      <- paste0(here::here(),"/Analysis_paper1/figTbl_paper1_v2/") 
...
df_PVC <- df_PVC_30y %>% filter(sim>=1) 
span <-  30
runs_select <-  runs_paper
...
df_PVC_baseline <- 
	...
	ungroup %>% select(sim, ERC_PV_baseline = ERC_PV, UAAL_PV_baseline = UAAL_PV, ERCwUAAL_PV_baseline = ERCwUAAL_PV)


# Values corresponding to the sim with median difference from the baseline
df_PVC_quintiles1 <-
	df_PVC %>% 
	select(runname, sim, ERC_PV, UAAL_PV, ERCwUAAL_PV) %>% 
  ...
  left_join(df_CAGR$df, by = "sim") %>% 
	...
	arrange(runname, grp_CAGR, ERC_PV_std_diff_pct) %>% 
	summarise(
		        
		        # median PV ERC of baseline
		        ERC_PV_baseline_med = median(ERC_PV_baseline),
		        
		        # Median difference from the baseline
		        ERC_PV_std_diff_pct = ERC_PV_std_diff_pct[round(n()/2)],
						
		        # Median values
		        ERC_PV_med  = median(ERC_PV),
		        UAAL_PV_med = median(UAAL_PV),
		        ERCwUAAL_PV_med = median(ERCwUAAL_PV), 
		        
		        
		        # Values under alt. policies corresponding to the median difference from the baseline
		        ERC_PV              = ERC_PV[round(n()/2)],
		        UAAL_PV             = UAAL_PV[round(n()/2)],
						ERCwUAAL_PV         = ERCwUAAL_PV[round(n()/2)],
		        
						
						# Values under baseline corresponding to the median difference from the baseline
						ERC_PV_baseline      = ERC_PV_baseline[round(n()/2)],
		        UAAL_PV_baseline     = UAAL_PV_baseline[round(n()/2)],
						ERCwUAAL_PV_baseline = ERCwUAAL_PV_baseline[round(n()/2)]
						) %>% 
  ...
  arrange(runname, desc(grp_CAGR)) %>% 
	...
	ungroup %>% 
	mutate(
		     fct_std   = ERC_PV_baseline_med[grp_CAGR == 3 & runname == "baseline"],
		     
				 a.ERC_PV_baseline_std      = 100 * ERC_PV_baseline      / fct_std,
				 b.UAAL_PV_baseline_std     = 100 * UAAL_PV_baseline     / fct_std,
				 c.ERCwUAAL_PV_baseline_std = 100 * ERCwUAAL_PV_baseline / fct_std,
				 
				 d.ERC_PV_std          = 100 * ERC_PV  / fct_std,
				 e.UAAL_PV_std         = 100 * UAAL_PV / fct_std,
				 f.ERCwUAAL_PV_std     = 100 * ERCwUAAL_PV / fct_std,
				 
				 g.ERC_PV_med_std      = 100 * ERC_PV_med      / fct_std,
				 h.UAAL_PV_med_std     = 100 * UAAL_PV_med     / fct_std,
				 i.ERCwUAAL_PV_med_std = 100 * ERCwUAAL_PV_med / fct_std,
				 
				 ERC_PV_baseline_med_std = 100 * ERC_PV_baseline_med / fct_std
				 )
...
## Making median-within-quintile table for ERC_

 
df_tbl_pvERC_med <- 
  df_PVC_quintiles1 %>% 
	select(runname, grp_CAGR, ERC_PV_baseline_med_std, ERC_pv_med_std = g.ERC_PV_med_std) %>% 
	filter(runname %in% names(runs_paper)) %>%
	arrange(runname, grp_CAGR) %>% 
	mutate(ERC_pv_med_std_diff = ERC_pv_med_std / ERC_PV_baseline_med_std - 1,
				 ERC_PV_baseline_med_std = NULL
				 ) %>% 
	gather(panel, value, -runname, -grp_CAGR) %>% 
	mutate(runname = factor(runname, labels = runs_paper),
				 panel   = factor(panel, 
				 								  levels = c("ERC_pv_med_std", "ERC_pv_med_std_diff"),
				 								  labels = c("Median present value of employer contribution within quintiles",
				 								  					 "Percentage difference from Baseline")
				 								 )) %>% 
	spread(runname, value) %>% 
	arrange(panel, grp_CAGR) %>% 
	mutate(Return_Range = rep(df_CAGR$cutoff, 2)) %>% 
  relocate(panel, grp_CAGR, Return_Range) %>% 
	group_by(panel)
...
tbl_ERC_PV_med <-
  df_tbl_pvERC_med %>% 
	...
	tab_header(
    title = html("30-year present value of employer contributions under difference policies<br> 
by long-term return quintile")
  ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
		Return_Range = "Range of 30-year compound returns" 
	) %>% 
...
tbl_ERC_PV_med
gtsave(tbl_ERC_PV_med, filename = paste0(dir_outputs, "tbl_ERC_PV_med.pdf"))
#gtsave(tbl_ERC_PV_med, filename = paste0(dir_outputs, "bl_ERC_PV_med.rtf"))	
	
	
	
	
##  Data for making table for median difference from baseline
df_tab_ERC_PV <- 
df_PVC_quintiles1 %>% 
	select(runname, grp_CAGR , ERC_PV_std_diff_pct) %>% 
	spread(runname, ERC_PV_std_diff_pct) %>% 
	# select(grp_CAGR, one_of(runnames_show1)) %>% 
	left_join(df_PVC_quintiles1 %>%  # adding median baseline PV ERC in each group
							filter(runname == "baseline") %>% 
							ungroup() %>% 
							select(grp_CAGR, ERC_PV_std_baseline = ERC_PV_baseline_med_std), 
						by = "grp_CAGR") %>% 
...
# xlsx::write.xlsx2(df_tab_pvERC, file = "Analysis_paper1/figTbl_paper1/ERC_sim2sim.xlsx")
...
df_PVC_quintiles1 <-
  ...
  arrange(runname, desc(grp_CAGR)) %>% 
	#For paper: standardize using the group 3 value of baseline
	ungroup %>% 
	mutate(fct_std   = ERC_PV_baseline_med[grp_CAGR == 3 & runname == "baseline"],
...
##  Data for making table with detailed data, sim-to-sim comparison
df_tab_details <- 
df_PVC_quintiles1 %>% 
	filter(runname %in% names(runs_select)) %>% 
	select(runname, grp_CAGR , 
				 a.ERC_PV_baseline_std,     
				 b.UAAL_PV_baseline_std,    
				 c.ERCwUAAL_PV_baseline_std,
				 
				 d.ERC_PV_std,         
				 e.UAAL_PV_std,        
				 f.ERCwUAAL_PV_std  
) %>% 
	gather(Var, Value, -grp_CAGR, -runname) %>% 
	mutate(colName = paste(runname, Var, sep = "."),
				 runname = NULL, Var = NULL) %>% 
	spread(colName, Value) %>% 
	# select(grp_CAGR, one_of(runnames_show1)) %>% 
	# left_join(df_PVC_quintiles1 %>%  # adding median baseline PV ERC in each group
	# 						filter(runname == "baseline") %>% 
	# 						ungroup() %>% 
	# 						select(grp_CAGR, ERC_PV_std_baseline = ERC_PV_baseline_med), 
	# 					by = "grp_CAGR") %>% 
	# select(grp_CAGR, ERC_PV_std_baseline, everything(), -baseline) %>% 
	# gather(policy, value, -grp_CAGR) %>% 
	# # mutate(policy = factor(policy, levels = c("ERC_PV_baseline", runnames_show1),
...
xlsx::write.xlsx2(df_tab_details, file = "Analysis_paper1/figTbl_paper1/ERC_details.xlsx")


##  Data for making table, median within quintiles
df_tab_median <- 
df_PVC_quintiles1 %>% 
	filter(runname %in% names(runs_select)) %>% 
	select(runname, grp_CAGR, 
...
df_tab_ERC.med <- 
  df_tab_median %>% 
	select(grp_CAGR, Return_Range, contains("g.ERC"))

df_tab_UAAL.med <- 
  df_tab_median %>% 
	select(grp_CAGR, Return_Range, contains("h.UAAL"))

df_tab_ERCwUAAL.med <- 
  df_tab_median %>% 
	select(grp_CAGR, Return_Range, contains("i.ERCwUAAL"))


xlsx::write.xlsx2(df_tab_ERC.med,      file = "Analysis_paper1/figTbl_paper1/ERC_median.xlsx", sheetName = "ERC")
xlsx::write.xlsx2(df_tab_UAAL.med,     file = "Analysis_paper1/figTbl_paper1/ERC_median.xlsx", sheetName = "UAAL", append = TRUE)
xlsx::write.xlsx2(df_tab_ERCwUAAL.med, file = "Analysis_paper1/figTbl_paper1/ERC_median.xlsx", sheetName = "ERCwUAAL", append = TRUE)
...
tab_ERC_vol_gt <- 	
	df_tab_ERC_vol %>% 
		gt() %>% 
		tab_header(
    title = html(paste0("Maximum increase in employer contribution rate in 5 years <br>over a ", span, "-year period by long-term return quintile")),
    subtitle = "(Median values within return quintiles)"
    ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
		Return_Range = "Range of 30-year compound returns" 
...
# tbl_ERC_PV_med
# gtsave(tbl_ERC_PV_med, filename = paste0(dir_outputs, "bl_ERC_PV_med.pdf"))
...
tbl_ERCvol_y30_FR75$tbl_gt


gtsave(tbl_ERCvol_y30_FR75$tbl_gt, filename = paste0(dir_outputs, "tbl_ERC_vol_med.pdf"))
...
tbl_ERChike_FR75
gtsave(tbl_ERChike_FR75, paste0(dir_outputs, "tbl_ERC_hike_FR75.pdf"))
...
df_PVC_baseline <- 
	filter(df_PVC, runname == "baseline") %>% 
	ungroup %>% select(sim, EEC_PV_baseline = EEC_PV)
...
df_PVC_quintiles1 <-
  ...
  left_join(df_CAGR$df, by = "sim") %>% 
	...
	group_by(runname, grp_CAGR) %>% 
	arrange(runname, grp_CAGR, EEC_PV_std_diff_pct) %>% 
	summarise(
		        
		        # median PV EEC of baseline
		        EEC_PV_baseline_med = median(EEC_PV_baseline),
		        
		        # Median difference from the baseline
		        EEC_PV_std_diff_pct = EEC_PV_std_diff_pct[round(n()/2)],
						
		        # Median values
		        EEC_PV_med  = median(EEC_PV),

		        # Values under alt. policies corresponding to the median difference from the baseline
		        EEC_PV              = EEC_PV[round(n()/2)],

						# Values under baseline corresponding to the median difference from the baseline
						EEC_PV_baseline      = EEC_PV_baseline[round(n()/2)],
						) %>% 
  ...
  arrange(runname, desc(grp_CAGR)) %>% 
	...
	ungroup %>% 
	mutate(
		     fct_std   = EEC_PV_baseline_med[grp_CAGR == 3 & runname == "baseline"],
		     
				 a.EEC_PV_baseline_std      = 100 * EEC_PV_baseline      / fct_std,

				 d.EEC_PV_std          = 100 * EEC_PV  / fct_std,

				 g.EEC_PV_med_std      = 100 * EEC_PV_med      / fct_std,

				 EEC_PV_baseline_med_std = 100 * EEC_PV_baseline_med / fct_std
				 )
...
df_tbl_pvEEC_med <- 
  df_PVC_quintiles1 %>% 
	select(runname, grp_CAGR, EEC_PV_baseline_med_std, EEC_pv_med_std = g.EEC_PV_med_std) %>% 
	filter(runname %in% names(runs_paper)[c(1, 4, 5, 6, 7)]) %>%
	arrange(runname, grp_CAGR) %>% 
	mutate(EEC_pv_med_std_diff = EEC_pv_med_std / EEC_PV_baseline_med_std - 1,
				 EEC_PV_baseline_med_std = NULL
				 ) %>% 
	gather(panel, value, -runname, -grp_CAGR) %>% 
	mutate(runname = factor(runname, labels = runs_paper[c(1,4, 5, 6, 7)]),
				 panel   = factor(panel, 
				 								  levels = c("EEC_pv_med_std", "EEC_pv_med_std_diff"),
				 								  labels = c("Median present value of employer contribution within quintiles",
				 								  					 "Percentage difference from Baseline")
				 								 )) %>% 
	spread(runname, value) %>% 
...
tbl_EEC_PV_med <-
  df_tbl_pvEEC_med %>% 
	...
	tab_header(
    title = html("30-year present value of employee contributions under difference policies<br> 
by long-term return quintile")
  ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
...
tbl_EEC_PV_med
gtsave(tbl_EEC_PV_med, filename = paste0(dir_outputs, "tbl_EEC_PV_med.pdf"))
#gtsave(tbl_EEC_PV_med, filename = paste0(dir_outputs, "bl_ERC_PV_med.rtf"))	
...
get_EECvol_tbl <- function(df, span = NULL, runnames_select){
	...
	# span = 30
	# runnames_select = runs_paper[c(1, 4:7)]
...
tab_EEC_vol_gt <-  
	...
	gt() %>% 
		tab_header(
    title = html(paste0("Maximum increase in employee contribution rate in 5 years <br>over a ", span, "-year period by long-term return quintile")),
    subtitle = "(Median values within return quintiles)"
    ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
...
tbl_EECvol_y30_FR75$tbl_gt

gtsave(tbl_EECvol_y30_FR75$tbl_gt, filename = paste0(dir_outputs, "tbl_EEC_vol_med.pdf"))
...
df_pvB_diff_quintile %>% 
	...
	select(runname, grp_CAGR, B_PV, B_PV_diff_pct) %>% 
	ungroup() %>% 
	gather(Panel, value, -runname, -grp_CAGR) %>% 
	mutate(Panel = factor(Panel, levels = c("B_PV", "B_PV_diff_pct"),
										       labels = c("Median present value of benefit over 30 years within quintiles",
				 								  					 "Percentage difference from Baseline")
											)) %>% 
  ...
  # select(-baseline) %>% 
	...
	# 					) %>% 
	mutate(Return_Range = rep(df_CAGR$cutoff, 2) ) %>% 
	select(Panel, grp_CAGR, Return_Range, everything()) %>% 
	group_by(Panel)

ncol_tab <- ncol(df_tab_pvB)

# 1. The annual withdrawal rate from the DC balance is assumed to be 7.5%.
# 2. The present value of DC benefits is calculated as the sum of the present value of annual withdrawals from the DC account over age 60-85 and the present value of the remaining DC balance at the end of age 85.

tab_pvB_quintile_gt <- 
	df_tab_pvB %>% 
	gt() %>% 
		tab_header(
    title = html("Present value of benefit over 30 years (age 60-89):<br>Percentage differences from baseline by long-term return quintile"),
    subtitle = "Starting benefits are normalized to $100 for all policies"
  ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
		Return_Range = "Range of 30-year compound returns" 
	...
	# Footnotes
	 ...
	 tab_footnote(
    footnote = 'Annual benefit under the hybrid DB-DC plan is the sum of DB benefit and annual withdrawal from the DC balance. The annual withdrawal rate from the DC balance is assumed to be 7.5%. The present value of DC benefits is calculated as the sum of the present value of annual withdrawals from the DC account over age 60-85 and the present value of the remaining DC balance at the end of age 85. Under the " Hybrid DB-DC: high return" scenario, the DC assets held by retirees has the same returns and losses as the DB portfolio; under the " Hybrid DB-DC: low return" scenario, the DC assets held by retirees has one-half of the DB portfolio\'s returns and losses.',
    locations = cells_column_labels(
     c("Hybrid\nDB-DC: high return",  "Hybrid\nDB-DC: low return")
    )
 ) %>% 
...
tab_pvB_quintile_gt 
gtsave(tab_pvB_quintile_gt , filename = paste0(dir_outputs, "tbl_benefit_PV_med.pdf"))
...
tbl_Bvol_gt <- 
  df_tbl_Bvol %>% 
	gt() %>% 
		tab_header(
    title = html("Maximum 5-year decreases in real benefit by long-term return quintile"),
    subtitle = "(Median values within quintiles)"
  ) %>% 
	cols_label(
		grp_CAGR     = "Return quintile",
		Return_Range = "Range of 30-year compound returns" 
...
gtsave(tbl_Bvol_y30$tbl_gt, filename = paste0(dir_outputs, "tbl_benefit_vol_med.pdf"))
...
df_low_pvB %>% 
  ...
  gt(rowname_col = c("Measure")) %>% 
    ...
    decimals = 1) %>% 
	tab_header(
    title = html("Probability of present value of benefit below 90%, 95%, and 100%<br>of the level in baseline"), 
...
gtsave(tbl_Bpv_probLow, paste0(dir_outputs, "tbl_benefit_probLowPV.pdf"))
...
gtsave(tbl_B_real_probLow, paste0(dir_outputs, "tbl_benefit_probLowAnyYear.pdf"))
