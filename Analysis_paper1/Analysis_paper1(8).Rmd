...
## Table for PV ERC with final UAAL 
```{r tab_ERC_PV, echo = FALSE}

get_pvERCwUAAL_tbl <- function(df_PVC, span = NULL, runs_select){
...
df_PVC_ptiles <-
df_PVC %>%
	select(runname, sim, ERC_PV = ERCwUAAL_PV) %>%
	spread(runname, ERC_PV) %>%
	arrange(desc(baseline)) %>%
	mutate(sim_rank = 1:nsim) %>%
	filter(sim_rank %in% round(nsim*ptiles_measure)) %>%
...
df_PVC_baseline <- 
	filter(df_PVC, runname == "baseline") %>% 
	ungroup %>% select(sim, ERC_PV_baseline = ERCwUAAL_PV)
...
df_tab_ERC_PV2 <- 
df_PVC_quintiles %>% 
	select(runname, grp_CAGR , ERC_PV_std_diff) %>% 
	spread(runname, ERC_PV_std_diff) %>% 
	# select(grp_CAGR, one_of(runnames_show1)) %>% 
	left_join(df_PVC_quintiles %>%  # adding median baseline PV ERC in each group
							filter(runname == "baseline") %>% 
							ungroup() %>% 
...
fn_fmt <- function(x) formatC(as.numeric(x), format = 'f', flag='0', digits = 1)

df_tab_pvERC2 <- 
	df_tab_ERC_PV2 %>% 
	select(grp_CAGR, Return_Range, ERC_PV_std_baseline, one_of(names(runs_select))) %>% 
  gather(policy, value, -grp_CAGR, -Return_Range) %>% 
	mutate(value = ifelse(policy == "ERC_PV_std_baseline", fn_fmt(value),
												ifelse(value > 0, paste0("+", fn_fmt(value)),
															           fn_fmt(value)
															 ))) %>% 
	mutate(policy = factor(policy, levels = c("ERC_PV_std_baseline", 	names(runs_select) ),
												         labels = c("PV employer cost\nin Baseline", 	runs_select)),
...
tab_pvERC <- 
  df_tab_pvERC %>% 
	flextable() %>%
	add_header_row(values = paste0(span, "-year present value of employer contributions plus terminal UAAL under different policies:\n Percentage differences from baseline by long-term return quintile\n (Median differences within quintiles)"), 
								 colwidths = ncol_tab) %>% 
	# autofit() %>% 
	height(part = "body",   height = .6) %>%
	height(part = "header", i = 2, height = 1) %>%
...
tab_pvERC2 <- 
  df_tab_pvERC2 %>% 
	flextable() %>%
	add_header_row(values = paste0(span, "-year present value of employer contributions plus terminal UAAL under different policies:\n differences from baseline by long-term return quintile\n (Median differences within quintiles)"), 
								 colwidths = ncol_tab) %>% 
	# autofit() %>% 
	height(part = "body",   height = .6) %>%
	height(part = "header", i = 2, height = 1) %>%
	...
	# autofit(add_w = 1) %>% 
	align(j = 1:2, align = "center", part = "all")  %>% 
	align(j = 3:10, align = "right", part = "all")  %>% 
	#colformat_num(j = 4:ncol_tab, digits = 1, suffix = "%") %>% 
	#colformat_num(j = 3, digits = 1) %>% 
	set_header_labels(grp_CAGR = "Return\nquintile", 
										Return_Range = paste0("Range of ", span, "-year\ncompound returns"))
...
tbl_pvERCwUAAL_y30_FR75  <- get_pvERCwUAAL_tbl(df_PVC_30y, 30, runs_paper)
#tbl_pvERC_y30_FR100 <- get_pvERC_tbl(df_PVC_30y, 30, runs_paper_FR100)


tbl_pvERCwUAAL_y30_FR75$tbl
tbl_pvERCwUAAL_y30_FR75$tbl2
#tbl_pvERC_y30_FR100$tbl


save_as_image(tbl_pvERCwUAAL_y30_FR75$tbl, path = paste0(dir_outputs, "tbl_quintile_ERCwUAALpv_y30_FR75v1.png"))
save_as_image(tbl_pvERCwUAAL_y30_FR75$tbl2, path = paste0(dir_outputs, "tbl_quintile_ERCwUAALpv_y30_FR75v2.png"))
