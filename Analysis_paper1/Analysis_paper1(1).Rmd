...
tab_pvEEC <- 
  df_tab_pvEEC %>% 
	flextable() %>%
	add_header_row(values = paste0("Percentage differences in ", span, "-year PV employee contribution\n from baseline by long-term return quintile\n(Median values within groups)"), 
								 colwidths = ncol_tab) %>% 
	# autofit() %>% 
	height(part = "body",   height = .6) %>%
	height(part = "header", i = 2, height = 1) %>%
...
tbl_pvEEC_y30_FR75  <- get_pvEEC_tbl(df_PVC_30y, 30, runs_paper)
#tbl_pvERC_y30_FR100 <- get_pvERC_tbl(df_PVC_30y, 30, runs_paper_FR100)
...
tbl_EECvol_y30_FR75 <- get_EECvol_tbl(df_EECMaxChg_30y$EECMaxChg, 30, runs_paper)
