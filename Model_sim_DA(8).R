...
df_agg_sim$AA[1] <- df_agg_sim$MA[1]
# df_agg_sim$UAAL_fullIdx[1] <- df_agg_sim$AL_fullIdx[1] - df_agg_sim$MA[1]
df_agg_sim$UAAL_fullIdx[1] <- df_agg_sim$AL_fullIdx[1] - df_agg_sim$AA[1]



# Amortization costs
df_agg_sim$EUAAL[1] <- 0 # based on full idx
df_agg_sim$LG[1] <- df_agg_sim$AM[1] <-  with(df_agg_sim, UAAL_fullIdx[1])  # This is the intial underfunding, rather than actuarial loss/gain if the plan is established at period 1. 

if(amort_type == "closed") SC_amort[1, 1:(1 + m - 1)] <- amort_LG(df_agg_sim$AM[1], dr, m, salgrowth_amort, end = FALSE, method = amort_method)  

df_agg_sim$SC[1] <- switch(amort_type,
											     closed = sum(SC_amort[, 1]),
											     open   = amort_LG(df_agg_sim$UAAL_fullIdx[1], dr_DA, m, salgrowth_amort, end = FALSE, method = amort_method)[1]
													 )
...
df_agg_sim$EEC[1] <- with(df_agg_sim, PR[1] * EECrate_fixed)

df_agg_sim$ADC[1]    <- with(df_agg_sim, max(0, NC[1] + SC[1])) 
df_agg_sim$ADC.ER[1] <- with(df_agg_sim, ifelse(ADC[1] > EEC[1], ADC[1] - EEC[1], 0)) 

df_agg_sim$ERC[1] <- with(df_agg_sim, ADC.ER[1])

df_agg_sim$C[1] <- with(df_agg_sim, EEC[1] + ERC[1])

df_agg_sim$C_ADC[1] <- with(	df_agg_sim, C[1] - ADC[1])
...
# funded ratios TODO: move out of the loop?
df_agg_sim$FR_MA_fullIdx[1]    <- with(df_agg_sim, MA[1] / AL_fullIdx[1])
df_agg_sim$FR_MA_floorIdx[1]   <- with(df_agg_sim, MA[1] / AL_floorIdx[1])

df_agg_sim$FR_AA_fullIdx[1]    <- with(df_agg_sim, AA[1] / AL_fullIdx[1])
df_agg_sim$FR_AA_floorIdx[1]   <- with(df_agg_sim, AA[1] / AL_floorIdx[1])



# index, based on actuarial value of assets
...
if(init_UAAL_type != "model"){
	...
	AL_year1 <- df_year1$AL_fullIdx 
	MA_year1 <- df_year1$MA
	...
	df_agg_sim$SC_legacy <- SC_amort_legacy[1:nyear]
...
for(j in 2:nyear){
  ...
  # 1. Determining MA and AA based on the cash flows in previous year
	...
	df_agg_sim$AA[j]  <- switch(smooth_method,
													    method1 = with(df_agg_sim, MA[j] - sum(s.vector[max(s.year + 2 - j, 1):s.year] * I.diff[(j-min(j, s.year + 1)+1):(j-1)])),
													    method2 = with(df_agg_sim, MA[j]) 
	)
  ...
  # 2. Calculating nominal and real liabilities 	
	...
	df_agg_sim$UAAL_fullIdx[j] <- df_agg_sim$AL_fullIdx[j] - df_agg_sim$AA[j]
	

	# 3. Calculating nominal and real funded ratio
	df_agg_sim$FR_MA_fullIdx[j]  <- with(df_agg_sim, MA[j] / AL_fullIdx[j])
	df_agg_sim$FR_MA_floorIdx[j] <- with(df_agg_sim, MA[j] / AL_floorIdx[j])

	df_agg_sim$FR_AA_fullIdx[j]  <- with(df_agg_sim, AA[j] / AL_fullIdx[j])
	df_agg_sim$FR_AA_floorIdx[j] <- with(df_agg_sim, AA[j] / AL_floorIdx[j])
  ...
  # df_agg_sim 
	...
	df_agg_sim$EUAAL[j] <- with(df_agg_sim, (UAAL_fullIdx[j - 1] + NC[j - 1])*(1 + dr_DA) - C[j - 1] * (1 + dr_DA)) # - Ic[j - 1] )
	df_agg_sim$LG[j]    <- with(df_agg_sim,  UAAL_fullIdx[j] - EUAAL[j])
	df_agg_sim$AM[j]    <- with(df_agg_sim,  LG[j] - (C_ADC[j - 1]) * (1 + dr_DA))
	
	
	if(amort_type == "closed") SC_amort[j, j:(j + m - 1)] <- amort_LG(df_agg_sim$AM[j], dr, m, salgrowth_amort, end = FALSE, method = amort_method)  
	
	df_agg_sim$SC[j] <- switch(amort_type,
														 closed = sum(SC_amort[, j]),
														 open   = amort_LG(df_agg_sim$UAAL_fullIdx[j], dr_DA, m, salgrowth_amort, end = FALSE, method = amort_method)[1]
	                           )
